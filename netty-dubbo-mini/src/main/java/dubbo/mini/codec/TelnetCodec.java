...
public class TelnetCodec extends TransportCodec {
  ...
  private static Charset getCharset(NetChannel channel) {
    if (channel != null) {
  ...
  protected Object decode(NetChannel channel, ChannelBuffer buffer, int readable, byte[] message)
      throws IOException {
    if (isClientSide(channel)) {
    ...
    if (message[message.length - 1] == '\b') { // Windows backspace echo
      try {
        boolean doublechar =
            message.length >= 3 && message[message.length - 3] < 0; // double byte char
    ...
    for (Object command : EXIT) {
      if (isEquals(message, (byte[]) command)) {
        if (logger.isInfoEnabled()) {
          logger.info("", new Exception("Close channel " + channel + " on exit command: " + Arrays
    ...
    for (Object command : EXIT) {
      if (isEquals(message, (byte[]) command)) {
        if (logger.isInfoEnabled()) {
          logger
              .info("", new Exception("Close channel " + channel + " on exit command " + command));
        }
        channel.close();
        return null;
      }
    ...
    if (CollectionUtils.isNotEmpty(history) && index != null && index >= 0 && index < history
        .size()) {
      String value = history.get(index);
      if (value != null) {
