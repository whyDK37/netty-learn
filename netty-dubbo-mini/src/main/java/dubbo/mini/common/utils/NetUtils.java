...
public class NetUtils {
   ...
   * <p>
   * The %5 after ipv6 address is called scope id. see java doc of {@link Inet6Address} for more
   * details.
   *
  ...
  public static void setInterface(MulticastSocket multicastSocket, boolean preferIpv6)
      throws IOException {
   ...
   */
  public static boolean matchIpRange(String pattern, String host, int port)
      throws UnknownHostException {
    ...
    for (int i = 0; i < mask.length; i++) {
      ...
      } else if ("0".equals(ip_address[i]) && ("0".equals(mask[i]) || "00".equals(mask[i]) || "000"
  ...
  private static void checkHostPattern(String pattern, String[] mask, boolean isIpv4) {
    if (!isIpv4) {
      if (mask.length != 8 && ipPatternContainExpression(pattern)) {
        throw new IllegalArgumentException(
            "If you config ip expression that contains '*' or '-', please fill qulified ip pattern like 234e:0:4567:0:0:0:3d:*. ");
      }
