...
public class Constants {
  ...
  public static final String DUBBO = "zt";
  ...
  /**
   * By default, a consumer JVM instance and a provider JVM instance share a long TCP connection
   * (except when connections are set), which can set the number of long TCP connections shared to
   * avoid the bottleneck of sharing a single long TCP connection.
   */
  ...
  /**
   * key for router type, for e.g., "script"/"file",  corresponding to ScriptRouterFactory.NAME,
   * FileRouterFactory.NAME
   */
  ...
  /**
   * Every heartbeat duration / HEATBEAT_CHECK_TICK, check if a heartbeat should be sent. Every
   * heartbeat timeout duration / HEATBEAT_CHECK_TICK, check if a connection should be closed on
   * server side, and if reconnect on client side
   */
  ...
  /**
   * To decide whether register center saves file synchronously, the default value is
   * asynchronously
   */
  ...
  /**
   * when ROUTER_KEY's value is set to ROUTER_TYPE_CLEAR, RegistryDirectory will clean all current
   * routers
   */
  ...
  public static final String[] DEFAULT_REGISTER_PROVIDER_KEYS = {APPLICATION_KEY, CODEC_KEY,
      EXCHANGER_KEY, SERIALIZATION_KEY, CLUSTER_KEY, CONNECTIONS_KEY, DEPRECATED_KEY,
      GROUP_KEY, LOADBALANCE_KEY, MOCK_KEY, PATH_KEY, TIMEOUT_KEY, TOKEN_KEY, VERSION_KEY,
      WARMUP_KEY, WEIGHT_KEY, TIMESTAMP_KEY, RELEASE_KEY};

  public static final String[] DEFAULT_REGISTER_CONSUMER_KEYS = {APPLICATION_KEY, VERSION_KEY,
