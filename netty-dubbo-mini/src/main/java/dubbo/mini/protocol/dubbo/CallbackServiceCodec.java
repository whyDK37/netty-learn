...
class CallbackServiceCodec {
    ...
    public static Object encodeInvocationArgument(NetChannel channel, RpcInvocation inv, int paraIndex) throws IOException {
        // get URL directly
    ...
    private static String exportOrUnexportCallbackService(NetChannel channel, NetURL url,
        ...
        NetURL exportUrl = new NetURL(DubboProtocol.NAME, channel.getLocalAddress().getAddress().getHostAddress(), channel.getLocalAddress().getPort(), clazz.getName() + "." + instid, tmpMap);
    ...
    private static boolean isInstancesOverLimit(NetChannel channel, NetURL url, String interfaceClass, int instid, boolean isServer) {
    ...
    public static Object decodeInvocationArgument(NetChannel channel, RpcInvocation inv, Class<?>[] pts, int paraIndex, Object inObject) throws IOException {
    ...
    private static String getServerSideCallbackInvokerCacheKey(NetChannel channel, String interfaceClass, int instid) {
        return getServerSideCallbackServiceCacheKey(channel, interfaceClass, instid) + "." + "invoker";
    }

    private static String getServerSideCallbackServiceCacheKey(NetChannel channel, String interfaceClass, int instid) {
        return Constants.CALLBACK_SERVICE_PROXY_KEY + "." + System.identityHashCode(channel) + "." + interfaceClass + "." + instid;
    }

    private static Object referOrDestroyCallbackService(NetChannel channel, NetURL url, Class<?> clazz, Invocation inv, int instid, boolean isRefer) {
        Object proxy = null;
        ...
        if (isRefer) {
            if (proxy == null) {
                NetURL referurl = NetURL.valueOf("callback://" + url.getAddress() + "/" + clazz.getName() + "?" + Constants.INTERFACE_KEY + "=" + clazz.getName());
