...
/**
 * This is a very simple example for an HTTPS client that uses OCSP stapling. The client connects to
 * an HTTPS server that has OCSP stapling enabled and then uses BC to parse and validate it.
 */
public class OcspClientExample {
  ...
  private static ChannelInitializer<Channel> newClientHandler(
    ...
    return new ChannelInitializer<Channel>() {
      ...
      public void channelInactive(ChannelHandlerContext ctx) throws Exception {
        if (!promise.isDone()) {
          promise
              .tryFailure(new IllegalStateException("Connection closed and Promise was not done."));
        }
